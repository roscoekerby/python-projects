...
def calculate_LAXI_FIT(length_std, length_mean, angle_std, crossing_fraction, num_inliers, total_matches,
    ...
    # Apply too few inlier penalties

    # Ensure final_score does not exceed 0.5 when num_inliers < min_inliers
    if num_inliers < min_inliers:
        final_score = min(base_score, 0.5 * (num_inliers / min_inliers) * base_score)
...
def identify_with_LAXI_FIT(query_image, train_image, visualize=False):
    """
    ...
    print(f"LAX-FIT Score: {LAXI_FIT_score:.2f}")

    return LAXI_FIT_score

# Example usage:

image1_path = r'path_to_image.jpg'
image2_path = r'path_to_image2.jpg'
...
identify_with_LAXI_FIT(img1, img2, visualize=True)
